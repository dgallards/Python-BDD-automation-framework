...
environment = settings.PROFILES['environment']  # Use the imported environment setting
...
def step_when_prepare_body_request(context, file_name):
    env_folder = f"settings/environments/{environment}"
    file_path = f"{env_folder}/{file_name}"
    with open(file_path, 'r') as file:
        context.request_data = file.read()
    log(context, f"Prepared request body from file: {file_path}")
